...
title1 <- url1_ %>%
   html_nodes("span") %>%
  html_text()
t1 <- title1 [c(72,81,123,141,159,177, 195,213 ,231, 249,257, 280,298)] 

title2 <- url1_.1 %>%
   html_nodes("span") %>%
  html_text()
t2 <- title2   [c(72,81,123,141,159,177, 195,213 ,232, 250, 268,286)]  

title3 <- url1_.2 %>%
   html_nodes("span") %>%
  html_text()
t3 <- title3 [c(72,81,123,141,159,175, 194,213 ,232, 251, 268,287)] 
t3 <- t3 %>% str_trim()

title4<- url1_.3%>%
   html_nodes("span") %>%
  html_text()
t4 <- title4  [c(72,81,123,141,159,177, 185,204 ,220, 239, 247,266,285, 303)] 
...
selected_datatitle <- head(datatitle, 50)
...
combinedproduct1 <- data.frame(  
        Product = pro,
        Title_of_Review = selected_datatitle,
...
title2 <- url2_ %>%
   html_nodes("span") %>%
  html_text()
title2 <- title2 [c(71,82,119,127,147,165,173,192,210, 218,236 ,254, 273)] 
title2 <- title2 %>% str_trim()

title2.1 <- url2_.1 %>%
   html_nodes("span") %>%
  html_text()
title2.1 <- title2.1   [c(71,82,126,147,164,182,192,210,230,251,269,283 ,300)] 
title2.1 <- title2.1 %>% str_trim()

title2.2 <- url2_.2 %>%
   html_nodes("span") %>%
  html_text()
title2.2 <- title2.2 [c(71,81,123,141,159, 175,194 ,213, 232, 251,269, 287)] 
title2.2 <- title2.2 %>% str_trim()
title2.2

title2.3<- url2_.3%>%
   html_nodes("span") %>%
  html_text()
title2.3 <- title2.3  [c(71,82,127,147,156,171,179, 207,227 ,243, 261, 278,293, 307)]
title2.3 <- title2.3 %>% str_trim()
title2.3
```
```{r} 
#type of purhcase 
...
titleRev <- data.frame(
  TitleOfReview = c(title2,title2.1,title2.2,title2.3)[1:50]
)
...
combinedproduct2 <- data.frame( 
   Product = pro2,
   TitleOfreview = titleRev,
   ReviewerName = dataname2,
   TypeOfPurchase = datapurchaseType2,
   Date = dataDate2,
   IndividualRating = rate2,
    ReviewText = review_dataframe2
)
...
title3 <- url3_ %>%
   html_nodes("span") %>%
  html_text()
title3 <- title3 [c(72,83,125,144,163,181, 199,217 ,235, 253,271, 289)]  %>% str_trim()


title3.1 <- url3_.1 %>%
   html_nodes("span") %>%
  html_text()
title3.1 <- title3.1   [c(72,83,125,144,162,180, 198,217 ,235, 253, 271,289)]  %>% str_trim()

title3.2 <- url3_.2 %>%
   html_nodes("span") %>%
  html_text()
title3.2 <- title3.2 [c(72,83,125,149,167,185, 204,222 ,240, 258, 276,295)] 
title3.2 <- title3.2 %>% str_trim()

title3.3<- url3_.3%>%
   html_nodes("span") %>%
  html_text()
title3.3 <- title3.3  [c(72,83,125,144,162,180, 198,216 ,234, 252, 260,278,297)] 

title3.3 <- title3.3 %>% str_trim()
...
TitleRev3 <- data.frame(
    TitleOfReview = c(title3, title3.1, title3.2, title3.3)[1:50]
)
TitleRev3
...
combinedproduct3 <- data.frame( 
          Product = pro3,
          TitleOfReview = TitleRev3,
          ReviewerName = dataname3,
          TypeOfPurchase = datapurchaseType3,
          Date = dataDate3,
          IndividualRating = rate3,
          ReviewText = review_dataframe3
)
...
# Name/Titleofreview/Type/Date/Individual/Review (50)
...
title4 <- url4_ %>%
   html_nodes("span") %>%
  html_text()
title4 <- title4 [c(72,81,126,144,162,180,188,206,224,246,263,277,285,299)]  %>% str_trim()


title4.1 <- url4_.1 %>%
   html_nodes("span") %>%
  html_text()
title4.1 <- title4.1   [c(72,83,125,144,162,180, 198,217 ,235, 253, 271,289)]  %>% str_trim()

title4.2 <- url4_.2 %>%
   html_nodes("span") %>%
  html_text()
title4.2 <- title4.2 [c(72,81,130,147,161,175, 183,200 ,220, 240, 260,280,297)] %>% str_trim()

title4.3<- url4_.3%>%
   html_nodes("span") %>%
  html_text()
title4.3 <- title4.3  [c(72,81,127,142,157,174, 191,208 ,218, 236, 256,273,291)]  %>% str_trim()
...
title4.4<- url4_.4%>%
   html_nodes("span") %>%
  html_text()
title4.4 <- title4.4  [c(72,81,126,130,151,168, 185,205 ,222, 240, 257,274,294)]  %>% str_trim()
...
TitleRev4 <- data.frame(
  TitleOfReview =c(title4, title4.1, title4.2, title4.3, title4.4)[1:50]
)
...
combinedproduct4 <- data.frame( 
  Product = pr4,
  TitleOfreview = TitleRev4,
  ReviewerName = dataname4, 
  TypeOfPurchase = datapurchaseType4,
  Date = dataDate4,
  IndividualRating = rate4,
  ReviewText = review_dataframe4
)
...
title5 <- url5_ %>%
   html_nodes("span") %>%
  html_text()
title5 <- title5 [c(108,126,144,166,187,208,225,243,263,280)]  %>% str_trim()
title5

title5.1 <- url5_.1 %>%
   html_nodes("span") %>%
  html_text()
title5.1 <- title5.1   [c(108,126,144,166,187,208 ,225, 243, 263,280)]  %>% str_trim()

title5.2 <- url5_.2 %>%
   html_nodes("span") %>%
  html_text()
title5.2 <- title5.2 [c(112,133,150, 168,188 ,209, 230, 250,175,291)] %>% str_trim()

title5.3<- url5_.3%>%
   html_nodes("span") %>%
  html_text()
title5.3 <- title5.3  [c(112,132,142,160,177, 195,216 ,233, 251, 271,281,301)]  %>% str_trim()


title5.4<- url5_.4%>%
   html_nodes("span") %>%
  html_text()
title5.4 <- title5.4  [c(112,132,149, 163,181 ,202, 222, 243,264,281)]  %>% str_trim()
...
TitleRev5 <- data.frame(
  TitleOfReview = c(title5, title5.1, title5.2, title5.3, title5.4)[1:50]
)
TitleRev5
...
combinedproduct5 <- data.frame( 
  Product = pro5,
  TitleOfReview = TitleRev5,
  ReviewerName = dataname5,
  TypeOfPurchase = datapurchaseType5,
  Date = dataDate5,
  IndividualRating = rate5,
  ReviewText = review_dataframe5
)
...
title6 <- url6_ %>%
   html_nodes("span") %>%
  html_text()
title6 <- title6 [c(71,82,124,132,150,176,184,206,227,248,268,286,303,323)]  %>% str_trim()
title6

title6.1 <- url6_.1 %>%
   html_nodes("span") %>%
  html_text()
title6.1 <- title6.1   [c(71,82,124,143,151,173,190,218 ,232, 250, 271,289)]  %>% str_trim()

title6.2 <- url6_.2 %>%
   html_nodes("span") %>%
  html_text()
title6.2 <- title6.2 [c(71,82,124,143,166,186, 196,212 ,229, 247, 265,282,299)] %>% str_trim()

title6.3<- url6_.3%>%
   html_nodes("span") %>%
  html_text()
title6.3 <- title6.3  [c(71,82,124,143,151,173,190,218 ,232, 250, 271,289)]  %>% str_trim()
 #resume here
